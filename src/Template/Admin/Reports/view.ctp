									...
									<div class="widget-body no-padding">
										...
										<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
											...
											<?php foreach($jobs as $value) {
												if($value['scheduled_service']['is_done'] == 'Done') {
													$created = date("Y-m-d",strtotime($value['scheduled_service']['created_at']));
														if($created <= $Date) { ?>
													<tr>
														<td><?php echo h(date("Y-M-d",strtotime($value['scheduled_service']['created_at']))); ?></td>
